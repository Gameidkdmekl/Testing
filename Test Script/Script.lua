...
function CreateBillboardESP(Name, Part, Color, TextSize)
    ...
    TextLabel.Text = "Loading..."  -- Временный текст
...
NextbotToggle:OnChanged(function(value)
    if value then
        ...
        -- Немедленно запускаем сканирование при включении
        task.spawn(function()
            for i = 1, 3 do  -- Сканируем несколько раз для надежности
                scanForNextbots()
                task.wait(0.1)
            end
        end)
        ...
        Fluent:Notify({
            Title = "Nextbot ESP",
            Content = "Nextbot ESP enabled! Scanning for targets...",
            Duration = 3
        })
    else
        if nextbotLoop then

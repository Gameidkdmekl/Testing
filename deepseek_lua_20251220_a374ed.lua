...
local fGetHwid = gethwid or function() 
    -- Используем UserId как HWID для Platoboost
...
--! Проверка ключа через Platoboost API
    ...
    if response.StatusCode == 200 then
        ...
        if decoded and decoded.success == true then
            if decoded.data.valid == true then
                ...
                -- Если ключ не в whitelist, пробуем активировать его
                if fStringSub(key, 1, 4) == "KEY_" then
                    ...
                    if redeemResponse and redeemResponse.StatusCode == 200 then
                        local decodedRedeem = lDecode(redeemResponse.Body)
                        
                        if decodedRedeem and decodedRedeem.success == true then
                            if decodedRedeem.data.valid == true then
                                if useNonce then
                                    if decodedRedeem.data.hash == lDigest("true" .. "-" .. nonce .. "-" .. secret) then
                                        return true, "Key activated and verified!"
                                    else
                                        return false, "Activation security failed"
                                    end
                                else
                                    return true, "Key activated and verified!"
                                end
...
--- НАСТРОЙКА КНОПОК ДЛЯ PLATOBOOST ---

-- Кнопка Submit (Проверка ключа через Platoboost)
submitBtn.MouseButton1Click:Connect(function()
    print("[Platoboost] Submit button clicked")
...
-- Кнопка Get Key (Получение ссылки от Platoboost)
getKeyBtn.MouseButton1Click:Connect(function()
    print("[Platoboost] Get Key button clicked")
...
task.spawn(function()
    task.wait(1)
    showMessage("Click 'Get Key' to get Platoboost access link", Color3.fromRGB(255, 255, 255))
end)
...
-- Предзагрузка ссылки Platoboost при запуске
task.spawn(function()
    task.wait(2)
...
print("Host:", host)
print("Nonce enabled:", useNonce)
print("=======================================")
print("Buttons configured for Platoboost:")
print("1. Get Key → Gets Platoboost link")
print("2. Submit → Verifies key via Platoboost API")
print("=======================================")
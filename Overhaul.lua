...
local function createPlayerHighlight(character)
    ...
    highlight.FillColor = Color3.new(0, 1, 0) -- Зеленый для обычных игроков
...
local function createPlayerBillboardGui(character, player)
...
local function updatePlayerBillboardInfo(textLabel, player)
    ...
    if revivesFolder then
        extraText = " | Revives"
        textLabel.TextColor3 = Color3.new(1, 1, 0) -- Желтый для Revives
    else
        textLabel.TextColor3 = Color3.new(0, 1, 0) -- Зеленый для обычных
    end

    textLabel.Text = string.format("%s | %dm%s", player.Name, distance, extraText)
end

-- Обновляем цвет Highlight в зависимости от наличия Revives
local function updatePlayerHighlightColor(highlight, character)
    if character:FindFirstChild("Revives") then
        highlight.FillColor = Color3.new(1, 1, 0) -- Желтый
        highlight.OutlineColor = Color3.new(1, 1, 0)
    else
        highlight.FillColor = Color3.new(0, 1, 0) -- Зеленый
        highlight.OutlineColor = Color3.new(0, 1, 0)
    end
end
...
local function setupPlayerHighlight(player)
    ...
    if playerBillboardGuis[player] then
        if playerBillboardGuis[player].Parent then
            playerBillboardGuis[player]:Destroy()
        end
        playerBillboardGuis[player] = nil
    end

    local highlight = createPlayerHighlight(character)
    playerHighlights[player] = highlight

    local billboardGui, textLabel = createPlayerBillboardGui(character, player)
    if billboardGui then
        playerBillboardGuis[player] = billboardGui
    end

    local function onRevivesChanged()
        updatePlayerHighlightColor(highlight, character)
    end
...
-- Функция для сканирования игроков (для интеграции с существующей системой)
...
-- Обновление дистанции каждый кадр (интеграция с существующей системой)
local function updatePlayerESPLoop()
    if not LocalPlayer.Character then
        return
    end
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        ...
        if player.Character and playerBillboardGuis[player] then
            local textLabel = playerBillboardGuis[player]:FindFirstChild("InfoLabel")
            if textLabel then
                updatePlayerBillboardInfo(textLabel, player)
            end
        end
    end
end
...
PlayerToggle:OnChanged(function(value)
    if value then
        if not playerLoop then
            playerLoop = RunService.RenderStepped:Connect(function()
                if Options.PlayerToggle.Value then
                    -- Включаем нашу систему ESP игроков
                    scanForPlayers()
                    updatePlayerESPLoop()
                end
            end)
        ...
        for player, billboardGui in pairs(playerBillboardGuis) do

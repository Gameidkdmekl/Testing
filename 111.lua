...
-- Кнопка Delete Key (для удаления сохраненного ключа)
local deleteKeyBtn = Instance.new("TextButton")
deleteKeyBtn.Size = UDim2.new(0, 150, 0, 30)
deleteKeyBtn.Position = UDim2.new(0.5, -75, 0, 285)
deleteKeyBtn.BackgroundColor3 = Color3.fromRGB(100, 100, 100)
deleteKeyBtn.Text = "Delete Saved Key"
deleteKeyBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
deleteKeyBtn.Font = Enum.Font.SourceSans
deleteKeyBtn.TextSize = 18
deleteKeyBtn.Visible = false -- Показываем только если есть сохраненный ключ
deleteKeyBtn.Parent = mainFrame

local deleteKeyCorner = Instance.new("UICorner")
deleteKeyCorner.CornerRadius = UDim.new(0, 10)
deleteKeyCorner.Parent = deleteKeyBtn
...
local keyDataStore = DataStoreService:GetDataStore("DraconicHubKeys")
local player = game:GetService("Players").LocalPlayer
local savedKey = nil
local isKeyLoaded = false

-- Функция для загрузки сохраненного ключа
local function loadSavedKey()
    local success, loadedKey = pcall(function()
        return keyDataStore:GetAsync(tostring(player.UserId))
    end)
    
    if success and loadedKey then
        savedKey = loadedKey
        isKeyLoaded = true
        print("Загружен сохраненный ключ:", savedKey)
        return savedKey
    else
        print("Сохраненный ключ не найден или ошибка загрузки")
        return nil
    end
end
...
local function saveKey(key)
    ...
    if success then
        ...
        print("Ошибка сохранения ключа:", errorMsg)
        return false
    end
end

-- Функция для удаления сохраненного ключа
local function deleteSavedKey()
    ...
    if success then
        savedKey = nil
        deleteKeyBtn.Visible = false
        print("Сохраненный ключ удален")
        return true
    else
        print("Ошибка удаления ключа:", errorMsg)
        return false
    end
end
...
task.spawn(function()
    local loadedKey = loadSavedKey()
    if loadedKey then
        -- Заполняем поле ввода сохраненным ключом
        keyInput.Text = loadedKey
        
        -- Показываем кнопку удаления
        deleteKeyBtn.Visible = true
        
        -- Показываем сообщение
        showMessage("Сохраненный ключ загружен!", Color3.fromRGB(50, 255, 50))
        
        -- Автоматически проверяем ключ через 2 секунды
        task.wait(2)
...
local keyLink = "https://discord.gg/examplelink" -- ВСТАВЬ СВОЮ ССЫЛКУ ЗДЕСЬ!
...
submitBtn.MouseButton1Click:Connect(function()
    ...
    if isValidKey(enteredKey) then
        ...
        if saveSuccess then
            ...
            deleteKeyBtn.Visible = true -- Показываем кнопку удаления
        else
            showMessage("Ключ принят, но не сохранен! Запускаю Draconic Hub...", Color3.fromRGB(255, 200, 50))
        end
...
deleteKeyBtn.MouseEnter:Connect(function()
    deleteKeyBtn.BackgroundColor3 = Color3.fromRGB(255, 100, 100)
end)
...
-- Функционал кнопки удаления ключа
deleteKeyBtn.MouseButton1Click:Connect(function()
    if savedKey then
        local success = deleteSavedKey()
        if success then
            showMessage("Сохраненный ключ удален!", Color3.fromRGB(50, 200, 255))
            keyInput.Text = ""
        else
...
print("Система сохранения ключей активирована!")

...
local function makeRequest(options)
    ...
    local success, response = pcall(function()
        ...
        elseif options.Method == "POST" then
            return {
                Body = HttpService:PostAsync(options.Url, options.Body or ""),
                StatusCode = 200
            }
        end
    end)
...
local function executeScript()
    ...
    if not success then
        ...
        rotationSpeed = 0.5
        showMessage("Ошибка загрузки: " .. errorMsg, Color3.fromRGB(255, 50, 50))
    end
end
...
submitBtn.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        ...
        if success then
            showMessage("Key accepted! Loading...", Color3.fromRGB(50, 255, 50))
            rotationSpeed = 3
...
-- Код перетаскивания (оставьте ваш существующий код здесь)
-- ... (ваш код перетаскивания)
...
-- Для тестирования - временный ключ (удалите в продакшене)
task.spawn(function()
    task.wait(5)
    print("\n=== FOR TESTING ===")
    print("Submit button test: Click the Submit button")
    print("Get Key button test: Click the Get Key button")
    print("=================\n")
end)